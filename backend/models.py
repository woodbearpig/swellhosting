...
class SiteContent(Base):
    ...
    # Rotating secret that unlocks the real site while coming_soon_active is on.
    # Empty on first boot – auto-generated by the backend, and admin can rotate
    # from the "Preview link" panel in Admin → Coming Soon. Never returned from
    # the public /api/site-content endpoint – only from the auth-required
    # /api/admin/site-content – so it can't be scraped by visitors.
    preview_token: str = ""
