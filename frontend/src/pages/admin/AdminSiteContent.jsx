...
import { Save, Plus, Trash2, ArrowUp, ArrowDown, Type as TypeIcon } from 'lucide-react';
  ...
  return (
    <div className="space-y-6" data-testid="admin-site-content-page">
      ...
      <div className="card-cream p-6 space-y-4">
        ...
        {tab === 'typography' && (
          <div className="space-y-6" data-testid="admin-typography-tab">
            ...
            <div>
              ...
              <select
                ...
                value={data.font_serif_id || 'cormorant'}
                onChange={e => {
                  set({ font_serif_id: e.target.value });
                  applyFonts({ serifId: e.target.value, sansId: data.font_sans_id, scriptId: data.font_script_id });
                }}
                data-testid="admin-font-serif"
              >
                {FONT_PRESETS.serif.map(p => <option key={p.id} value={p.id}>{p.name}{p.preview ? ` — ${p.preview}` : ''}</option>)}
              </select>
              <div
                className="mt-3 card-cream p-5"
                style={{ fontFamily: (FONT_PRESETS.serif.find(p => p.id === (data.font_serif_id || 'cormorant')) || FONT_PRESETS.serif[0]).family }}
              >
            ...
            <div>
              ...
              <select
                className="input-cream"
                value={data.font_sans_id || 'manrope'}
                onChange={e => {
                  set({ font_sans_id: e.target.value });
                  applyFonts({ serifId: data.font_serif_id, sansId: e.target.value, scriptId: data.font_script_id });
                }}
                data-testid="admin-font-sans"
              >
                {FONT_PRESETS.sans.map(p => <option key={p.id} value={p.id}>{p.name}{p.preview ? ` — ${p.preview}` : ''}</option>)}
              </select>
              <div
                className="mt-3 card-cream p-5"
                style={{ fontFamily: (FONT_PRESETS.sans.find(p => p.id === (data.font_sans_id || 'manrope')) || FONT_PRESETS.sans[0]).family }}
              >
                ...
                <div className="mt-3 flex gap-2 text-sm">
                  <span className="btn-primary !h-9">Start inquiry</span>
                  <span className="btn-secondary !h-9">View gallery</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <select
                className="input-cream"
                value={data.font_script_id || 'allura'}
                onChange={e => {
                  set({ font_script_id: e.target.value });
                  applyFonts({ serifId: data.font_serif_id, sansId: data.font_sans_id, scriptId: e.target.value });
                }}
                data-testid="admin-font-script"
              >
                {FONT_PRESETS.script.map(p => <option key={p.id} value={p.id}>{p.name}{p.preview ? ` — ${p.preview}` : ''}</option>)}
              </select>
              <div
                className="mt-3 card-cream p-5 text-center"
                style={{ fontFamily: (FONT_PRESETS.script.find(p => p.id === (data.font_script_id || 'allura')) || FONT_PRESETS.script[0]).family }}
              >
                <p className="text-5xl leading-tight text-[color:var(--brand-sage-deep)]">a warm welcome</p>
              </div>
            </div>

            <div className="rounded-2xl bg-[color:var(--brand-sage-tint)] p-4 text-sm">
              <p className="font-medium mb-1">Tip: keep it to two families</p>
              <p className="text-[color:var(--brand-text-muted)]">A great pairing is one distinctive headline font + one clean body font + one delicate script for flourishes. Fonts load from Google Fonts so no upload is needed.</p>
            </div>
          </div>
        )}
        {tab === 'hero' && (
